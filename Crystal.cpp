...
void Crystal::contral()
{
	...
	if (_hp <= 0 && !_isStrongSpirit)
	{
		...
		if (!_motion->isPlay())
		{
			SOUNDMANAGER->play("크리스탈사망", GAMEMANAGER->getMusicVolume() * GAMEMANAGER->getMasterVolume());
			_motion->start();
		}
	...
	{
		if (!SOUNDMANAGER->isPlaySound("크리스탈 스킬"))
		{ 
			SOUNDMANAGER->play("크리스탈 스킬", GAMEMANAGER->getSfxVolume() * GAMEMANAGER->getMasterVolume());
		}
		_playerstate = SKILL;
